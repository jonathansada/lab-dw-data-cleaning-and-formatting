def cleanDataFrame(df):
    df = cleanColNames(df)
    df = cleanInvvalidValues(df)
    df = setDTypes(df)
    df = cleanNullValues(df)
    df = cleanDuplicated(df)

    return df

def cleanColNames(df):
    ncol = {"CUSTOMER": "customer_id", "ST":"state", "Customer Lifetime Value":"lifetime_value", "Monthly Premium Auto":"monthly_premium_fee", "Number of Open Complaints":"open_complaints"}
